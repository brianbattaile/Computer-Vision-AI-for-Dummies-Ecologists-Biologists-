...
I'm going to explain two different ways to annotate images, one is with LabelImg and the other is with DIVE.  DIVE is a more user friendly program than LabelImg and is probably the way to go, but it has an extra step to convert the format to Yolo Darknet.
...
DIVE can be found at https://kitware.github.io/dive/ and you can use either the online version or download it to your desktop.  There is an excellent guide on the website to walk you through how to use it.  The only real issue with DIVE is that it doesn't save in the yolo darknet format so we will need to convert the format it does give.  What we need to do is, once finished with annotating and image, click on the download button at the top, and then click on the "VIAME CSV" button.  Once finished with all the images you want to annotate, upen the DIVE_to_YoloDarkNet.R file, change the file paths and it should produce a Yolo Darknet annotation file for each VIAME CSV file.  
